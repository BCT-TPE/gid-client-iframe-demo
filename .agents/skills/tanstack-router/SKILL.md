...
When a route segment can have children, reserve the segment file itself for shared parent/layout
behavior and put the leaf page in an `index.tsx` file. For flat file routes in this repo, prefer
`app.workspace.shared.$sharedChatId.index.tsx` for the concrete
`/app/workspace/shared/$sharedChatId/` page, while
`app.workspace.shared.$sharedChatId.tsx` should be treated as the parent route that can render
shared loaders, guards, layout, and an `<Outlet />` for child routes.

This avoids accidentally mixing leaf-page UI with parent route behavior that also applies to
`/app/workspace/shared/$sharedChatId/$conversationId` and future children.
...
### Parent beforeLoad Runs First

In nested routes, parent route `beforeLoad` functions run before child route components mount.
If an OAuth callback route lives under a guarded auth parent like `/auth`, explicitly bypass the
parent unauthenticated guard for `/auth/callback`. Otherwise an already-authenticated callback
visit can redirect to `/app` before the callback component reads search params or runs effects.
